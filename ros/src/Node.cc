           ...
           image_transport::ImageTransport& image_transport)
{
  ...
  {
    node_handle_.param<std::string>(name_of_node_ + "/base_footprint_frame_id", base_footprint_frame_id_, "base_"
                                                                                                          "footprint");
    node_handle_.param<std::string>(name_of_node_ + "/robot_camera_frame_id", robot_camera_frame_id_, "torso_front_"
                                                                                                      "camera_link");
...
void Node::PublishPositionAsTransform(cv::Mat position)
{
  ...
  if (!correct_global_frame_)
  {
    ...
    tf_broadcaster.sendTransform(
        tf::StampedTransform(transform, current_frame_time_, map_frame_id_param_, camera_frame_id_param_));
    return;
  }
...
sensor_msgs::PointCloud2 Node::MapPointsToPointCloud(std::vector<ORB_SLAM2::MapPoint*> map_points)
{
  ...
  for (unsigned int i = 0; i < cloud.width; i++)
  {
    if (map_points.at(i)->nObs >= min_observations_per_point_)
    {
      ...
      data_array[1] = -1.0 * map_points.at(i)->GetWorldPos().at<float>(0);  // y. Do the transformation by just reading
                                                                            // at the position of x instead of y
      data_array[2] = -1.0 * map_points.at(i)->GetWorldPos().at<float>(1);  // z. Do the transformation by just reading
                                                                            // at the position of y instead of z
      // TODO dont hack the transformation but have a central conversion function for MapPointsToPointCloud and
      // TransformFromMat
